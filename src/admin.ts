...
import { initDb, query, sql } from './lib/dbsql';
...
async function init() {
  ...
  const foundFids = (await sql.query(query)).recordset;
  
  // map fid to track
  const obj: Record<string, any> = {};

  foundFids.forEach((track) => {
    obj[track.fid] = track;
  });
  ...
  for (let i = 0; i < Math.ceil(bulk.length / 1000); i++) {
    const query: string = `
      INSERT INTO listenEntries (started, ended, trackId)
      VALUES ${bulk.slice(i * 1000, Math.min((i + 1) * 1000, bulk.length)).map((le) => {
        return `('${le.started}', '${le.ended}', ${le.trackId})`
      }).join(',')}
    `;
...
async function pictures() {
  ...
  files = files.filter(f => formats.includes(path.extname(f.filepath).toLowerCase()));
...
async function test() {
  ...
  // loadTracks([String.raw`D:\Google Drive\Music`]);

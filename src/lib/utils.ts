...
export function stringifyId<T extends { id: any }>(obj: T): Omit<T, 'id'> & { id: string } {
    let ret = { ...obj };
...
export function stringifyIds(obj: Record<string, any>): any {
    if (Array.isArray(obj)) {
        return obj.map(o => stringifyIds(o));
...
export function filesFromDirectoryRS(dirPath: string): { filepath: string, stat: fs.Dirent }[]
{
    ...
    function getIt(_dirPath: string)
    {
        ...
        {
            const abs = path.join(dirPath, file.name);
            if (fs.statSync(abs).isDirectory())
            {
                files.push(...filesFromDirectoryRS(abs));
            }
            else
            {
                files.push({ filepath: abs, stat: file });
            }
        })
    }
